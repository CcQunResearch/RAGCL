...
def semisup_train(unsup_train_loader, train_loader, aug1, aug2, model, optimizer, device, lamda):
    ...
    for sup_data, unsup_data in zip(train_loader, unsup_train_loader):
        ...
        out1 = model(sup_data)
        sup_loss = F.nll_loss(out1, sup_data.y.long().view(-1))
        ...
        out2 = model.forward_graphcl(data)
        cl_loss = model.contrastive_loss(out2, data.y, device)

        list = []
        for item in data.to_data_list():
            if item.y.item() == nrlabel:
                list.append(item)
        data2 = Batch.from_data_list(list).to(device)
        aug_data2_1 = augment(data2, augs1)
        aug_data2_2 = augment(data2, augs2)
        aug_data2_1_out1 = model.forward_graphcl(aug_data2_1)
        aug_data2_2_out2 = model.forward_graphcl(aug_data2_2)
        nrloss = model.loss_graphcl(aug_data2_1_out1, aug_data2_2_out2)

        loss = sup_loss + 0.001 * cl_loss + 0.001 * nrloss
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        unlabel_dataset = TreeDataset(unlabel_dataset_path, word_embedding, word2vec, centrality, undirected)
