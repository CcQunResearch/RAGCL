...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        if args.model == 'ResGCN':
            model = ResGCN_graphcl(dataset=unlabel_dataset, num_classes=num_classes, hidden=args.hidden,
            ...
            model = BiGCN_graphcl(unlabel_dataset.num_features, args.hidden, args.hidden, num_classes).to(device)
        ...
        for k in ks:
            for r in range(ft_runs):
                ...
                for epoch in range(1, ft_epochs + 1):
                    ...
                    _ = fine_tuning(model, args.aug1, args.aug2, args.lamda, optimizer, train_loader, device)
