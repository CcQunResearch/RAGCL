...
def sup_train(train_loader, aug1, aug2, model, optimizer, device, lamda, use_unsup_loss):
    ...
    for data in train_loader:
        ...
        out = model(data)
        sup_loss = F.binary_cross_entropy(out, data.y.to(torch.float32))
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        model = ResGCN_graphcl(dataset=train_dataset, hidden=args.hidden, num_feat_layers=args.n_layers_feat,
        ...
        for epoch in range(1, epochs + 1):
            ...
            val_error, log_info, log_record = test_and_log(model, val_loader, test_loader, device, epoch,
